...
async fn everything_works(pool: sqlx::PgPool) -> Result<(), (StatusCode, String)> {
    ...
    let alice = create(
        &pool,
        UserDeclaration::new("alice", "alice@example.com", "pw"),
    )
    .await?;
    ...
    let john = create(
        &pool,
        UserDeclaration::new("john ", "john@example.com", "pw"),
    )
    .await?;
    ...
    assert!(create(
        &pool,
        UserDeclaration::new("bob", "robert@example.com", "pw")
    )
    .await
    .is_err());
    assert!(create(
    ...
    assert!(create(
        &pool,
        UserDeclaration::new("robert", "bob@example.com", "pw")
    )
    .await
    .is_err());
    assert!(create(
        &pool,
        UserDeclaration::new("bob", "bob+2@example.com", "pw")
    )
    .await
    .is_err());
    assert!(create(
        &pool,
        UserDeclaration::new("robert", "bob+2@example.com", "pw")
    )
    .await
    .is_ok());
    ...
    assert!(create(
        &pool,
        UserDeclaration::new("John Doe", "doe@exampe.com", "pw")
    )
    .await
    .is_err());
    assert!(create(
        &pool,
        UserDeclaration::new("JohnDoe", "doe@example.com", "pw")
    )
    .await
    .is_ok());
    ...
    assert!(amend(&pool, bob.user_id, UserAmendment::default())
        .await
        .is_ok());
